 ...
 */
export function runFfmpeg(args: string[], opts: RunFfmpegOptions = {}): Promise<{ stderrTail: string }> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = opts.timeoutMs
      ? setTimeout(() => {
          child.kill('SIGKILL');
          finish(new FfmpegError(`ffmpeg timed out after ${opts.timeoutMs} ms`, null, stderr.slice(-4000)));
        }, opts.timeoutMs)
      : undefined;
    const onAbort = () => {
      cancelled = true;
    ...
    child.on('error', (err) => finish(new FfmpegError(`ffmpeg failed to start: ${err.message}`, null, '')));
    child.on('close', (code) => {
 ...
 */
export async function writeConcatList(entries: readonly ConcatEntry[], listPath: string): Promise<void> {
...
export function composeMp4Args(input: ComposeMp4Input): string[] {
  ...
  args.push('-c:v', 'libx264', '-preset', preset, '-crf', crf, '-profile:v', 'high', '-pix_fmt', 'yuv420p');
  args.push('-movflags', '+faststart', '-t', (input.durationMs / 1000).toFixed(3), ...BITEXACT, input.outputPath);
  return args;
}
