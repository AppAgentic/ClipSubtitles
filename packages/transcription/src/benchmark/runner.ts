...
import { aggregateScores, evaluateGates, failedScore, scoreTranscript, type CaseScore, type GateResult, type ProviderAggregate } from './scoring';
...
export async function runBenchmark(opts: RunBenchmarkOptions): Promise<BenchmarkRun> {
  ...
  const baselineId = opts.baselineId ?? 'whisper';
  ...
  for (const c of cases) {
    ...
    for (const provider of providers) {
      for (let r = 0; r < repeats; r += 1) {
        if (opts.signal?.aborted) throw new ProviderError(provider.id, 'CANCELLED', 'Benchmark cancelled.');
        ...
        try {
          ...
          scores.push(
            scoreTranscript(truth, result.words, {
              ...
              latencyMs,
              durationMs,
              estimatedUsd: result.estimatedUsd ?? (provider.usdPerMinute !== null ? (durationMs / 60_000) * provider.usdPerMinute : null),
            }),
          );
          ...
          scores.push(
            failedScore(
              { caseId: c.id, category: c.category, providerId: provider.id, latencyMs: now() - started, durationMs },
              code,
              truth.words.length,
            ),
          );
  ...
  if (!live) {
    notes.push('Only mock providers ran. These numbers validate the harness and scorer; they are NOT evidence about any real provider.');
  }
  if (!aggregates.some((a) => a.providerId === baselineId)) {
    notes.push(`Baseline provider "${baselineId}" did not run; "better than baseline" gates are unevaluated.`);
  }
  return {
    startedAt,
