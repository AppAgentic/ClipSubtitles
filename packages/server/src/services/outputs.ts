...
import type { ExportRecord } from '@clipsubtitles/storage';
 ...
 */
export async function discardTaskOutputs(
  ctx: AppContext,
  task: { id: string; workspaceId: string },
): Promise<{ rows: number; blobs: number }> {
  const rows = await ctx.db.deleteExportsForTask(task.id);
...
export async function discardOutputsForTaskId(ctx: AppContext, taskId: string): Promise<void> {
  ...
  await discardTaskOutputs(ctx, task).catch((err) =>
    ctx.logger.warn('output cleanup failed', {
      taskId,
...
export async function publishOutputs(
  ...
  try {
    ...
    for (const file of files) {
      ...
      const s = await ctx.store.putFile(key, file.path, {
        move: true,
    ...
    return await ctx.db.transaction(async () => {
      ...
      for (const { file, key, bytes, sha256 } of stored) {
        rows.push(
          await ctx.db.createExport({
    ...
    await discardTaskOutputs(ctx, task).catch((cleanupErr) =>
      ctx.logger.warn('output cleanup after failed publish failed', {
        taskId: task.id,
