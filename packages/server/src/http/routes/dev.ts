...
import { DEFAULT_SEGMENTATION, computeContentHash, defaultStyle, segmentationForStyle } from '@clipsubtitles/core';
import { createAsset, createProject as createProjectRecord, getProject, updateAsset, updateProjectMeta } from '@clipsubtitles/storage';
...
export function registerDevRoutes(app: Hono<AppEnv>, ctx: AppContext): void {
  ...
  app.post('/dev/tokens', async (c) => {
    ...
    return c.json({ accessToken: minted.token, tokenType: 'Bearer', expiresAt: minted.expiresAt, subject, scopes: body.scopes ?? [...SCOPES] }, 201);
  });

  app.get('/dev/fixtures', (c) =>
    c.json({
      fixtures: BENCHMARK_CASES.filter((f) => f.demoVideo).map((f) => ({ id: f.id, title: f.title, language: f.language, available: existsSync(demoFixturePath(f.id)) })),
      hint: 'Run `pnpm fixtures:build` to generate demo clips.',
    }),
  );

  app.post('/dev/fixtures/:fixtureId/projects', authenticate(ctx, { modes: ['bearer', 'session'] }), async (c) => {
    ...
    if (!fixture || !existsSync(mp4)) throw new ApiError('NOT_FOUND', 'Fixture not found. Run `pnpm fixtures:build`.');
    ...
    const asset = createAsset(ctx.db, { workspaceId: principal.workspaceId, projectId: project.id, status: 'importing', origin: 'fixture', fileName: `${fixture.id}.mp4`, mimeType: 'video/mp4', now });
    ...
    const stored = await ctx.store.putFile(key, mp4);
    const truthKey = `${principal.workspaceId}/sources/${asset.id}/truth.json`;
    if (existsSync(`${mp4}.truth.json`)) {
      await ctx.store.putFile(truthKey, `${mp4}.truth.json`);
      updateAsset(ctx.db, asset.id, { truthKey }, now);
    }
    const ready = await finalizeSourceAsset(ctx, { ...asset, ...(existsSync(`${mp4}.truth.json`) ? { truthKey } : {}) }, { storageKey: key, bytes: stored.bytes, sha256: stored.sha256, mimeType: 'video/mp4' });
    audit(ctx, { principal, action: 'project.create', targetType: 'project', targetId: project.id, metadata: { origin: 'fixture', fixtureId, assetId: ready.id } });
    const fresh = getProject(ctx.db, principal.workspaceId, project.id);
    return c.json({ project: fresh ? buildProjectView(ctx, fresh, { includePages: false }) : null }, 201);
  });
  ...
  app.post('/dev/oauth/register', async (c) => {
    ...
    const client: DevClient = {
      clientId,
      clientName: typeof body.client_name === 'string' ? body.client_name.slice(0, 120) : 'Local MCP client',
      redirectUris: Array.isArray(body.redirect_uris) ? body.redirect_uris.filter((u) => typeof u === 'string').slice(0, 10) : [],
    };
    clients.set(clientId, client);
    return c.json({ client_id: clientId, client_name: client.clientName, redirect_uris: client.redirectUris, token_endpoint_auth_method: 'none', grant_types: ['authorization_code'], response_types: ['code'] }, 201);
  });
  ...
  app.get('/dev/oauth/authorize', (c) => {
    ...
    if (!p.clientId || !p.redirectUri || !p.codeChallenge || p.method !== 'S256') throw new ApiError('VALIDATION_FAILED', 'client_id, redirect_uri, and S256 code_challenge are required.');
    const hidden = ['client_id', 'redirect_uri', 'code_challenge', 'code_challenge_method', 'scope']
      .map((k) => `<input type="hidden" name="${k}" value="${(c.req.query(k) ?? '').replace(/"/g, '&quot;')}">`)
      .join('');
  ...
  app.post('/dev/oauth/authorize', async (c) => {
    ...
    const p = authorizeParams({ client_id: str('client_id'), redirect_uri: str('redirect_uri'), state: str('state'), code_challenge: str('code_challenge'), code_challenge_method: str('code_challenge_method'), scope: str('scope') });
    const subject = str('subject') ?? '';
    if (!MOCK_USERS.some((u) => u.subject === subject)) throw new ApiError('VALIDATION_FAILED', 'Unknown mock user.');
    if (!p.clientId || !p.redirectUri || !p.codeChallenge) throw new ApiError('VALIDATION_FAILED');
    const client = clients.get(p.clientId);
    if (client && client.redirectUris.length && !client.redirectUris.includes(p.redirectUri)) throw new ApiError('VALIDATION_FAILED', 'redirect_uri not registered.');
    const code = randomToken(24);
    codes.set(code, { subject, clientId: p.clientId, redirectUri: p.redirectUri, codeChallenge: p.codeChallenge, scope: p.scope.length ? p.scope : [...SCOPES], expiresAt: Date.now() + 5 * 60_000 });
  ...
  app.post('/dev/oauth/token', async (c) => {
    ...
    const form = contentType.includes('application/json') ? ((await c.req.json()) as Record<string, string>) : Object.fromEntries(Object.entries(await c.req.parseBody()).map(([k, v]) => [k, typeof v === 'string' ? v : '']));
    ...
    if (challenge !== record.codeChallenge) return c.json({ error: 'invalid_grant', error_description: 'PKCE verification failed' }, 400);
    if (form.client_id && form.client_id !== record.clientId) return c.json({ error: 'invalid_client' }, 400);
    if (form.redirect_uri && form.redirect_uri !== record.redirectUri) return c.json({ error: 'invalid_grant' }, 400);
    ...
    return c.json({ access_token: minted.token, token_type: 'Bearer', expires_in: ctx.config.auth.tokenTtlSeconds, scope: record.scope.join(' ') }, 200);
  });
}
