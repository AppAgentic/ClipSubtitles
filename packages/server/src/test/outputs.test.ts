...
import { getBalance, listExportsForTaskAll, type Db } from '@clipsubtitles/storage';
...
async function captionedProject(): Promise<CaptionProject> {
  const created = await h.api<CreateProjectResponse>('POST', '/v1/projects', { token, body: { title: 'Outputs test', fileName: 'clip.mp4' } });
  ...
  const gen = await h.api<{ task: Task }>('POST', `/v1/projects/${created.body.project.id}/captions`, { token, body: {} });
  expect(gen.status).toBe(202);
  await h.runTasks();
  const view = await h.api<CaptionProject>('GET', `/v1/projects/${created.body.project.id}`, { token });
  expect(view.body.status).toBe('captioned');
  return view.body;
}

async function approvedRender(project: CaptionProject, idempotencyKey: string): Promise<{ taskId: string; cost: number }> {
...
function failingExportsDb(db: Db, armed: () => boolean): Db {
  return new Proxy(db, {
    get(target, prop) {
      if (prop === 'prepare') {
        return (sql: string) => {
          if (armed() && /INSERT INTO exports/i.test(sql)) throw new TaskFailure('RENDER_FAILED', 'simulated export row failure', { retryable: true });
...
function workerWith(db: Db, workerId: string): TaskWorker {
  return new TaskWorker({ ...h.ctx, db }, { workerId, heartbeatMs: 50, leaseMs: 30_000, pollMs: 5, maintenanceEveryMs: 0, retentionEveryMs: 3_600_000 });
}
...
describe('export publish atomicity', () => {
  it('a failed export-row insert leaves no rows, no blobs, and no charge; the retry publishes exactly once', async () => {
    ...
    let armed = true;
    const failing = workerWith(failingExportsDb(h.ctx.db, () => armed), 'worker_failing');
    expect(await failing.runOnce()).toBe(true);

    const afterFail = await h.api<{ task: Task; exports?: Export[] }>('GET', `/v1/tasks/${taskId}`, { token });
    ...
    expect(listExportsForTaskAll(h.ctx.db, taskId)).toEqual([]);
    expect(await h.ctx.store.list(prefix)).toEqual([]);
    const mid = getBalance(h.ctx.db, workspaceId);
    expect(mid).toMatchObject({ available: before.available - cost, reserved: cost });

    armed = false;
    h.clock.advance(10 * 60_000);
    await h.runTasks();
    const done = await h.api<{ task: Task; exports: Export[] }>('GET', `/v1/tasks/${taskId}`, { token });
    ...
    for (const e of done.body.exports) expect(await h.ctx.store.exists(`${prefix}/${e.fileName}`)).toBe(true);
    expect(getBalance(h.ctx.db, workspaceId)).toMatchObject({ available: before.available - cost, reserved: 0 });
    const ledger = await h.api<{ entries: Array<{ kind: string; taskId?: string }> }>('GET', '/v1/credits/ledger', { token });
  ...
  it('a permanent failure after blobs were written discards them and releases the reservation', async () => {
    ...
    const permanent = new Proxy(h.ctx.db, {
      get(target, prop) {
        if (prop === 'prepare') {
          return (sql: string) => {
            if (/INSERT INTO exports/i.test(sql)) throw new TaskFailure('RENDER_FAILED', 'simulated permanent failure', { retryable: false });
            return target.prepare(sql);
          };
        }
        const value = Reflect.get(target, prop, target) as unknown;
    ...
    expect(await workerWith(permanent, 'worker_permanent').runOnce()).toBe(true);

    const failed = await h.api<{ task: Task; exports?: Export[] }>('GET', `/v1/tasks/${taskId}`, { token });
  ...
  it('discardOutputsForTaskId removes rows and blobs of a render task and ignores other kinds', async () => {
    ...
    await h.runTasks();
    const prefix = exportPrefix(workspaceId, taskId);
    expect(listExportsForTaskAll(h.ctx.db, taskId)).toHaveLength(2);
    expect(await h.ctx.store.list(prefix)).toHaveLength(2);
