...
.style-preview{display:grid;aspect-ratio:4/3;place-items:end center;border-radius:6px;background:#282522;padding:14px;color:#fff;font-weight:800;text-align:center;text-shadow:0 2px 5px #000}
...
.editor{display:grid;grid-template-columns:minmax(0,1fr);gap:0;overflow:hidden;border:1px solid var(--line);border-radius:8px;background:var(--bg)}
...
.stage-video{display:grid;min-height:320px;place-items:center;overflow:hidden;border-bottom:1px solid var(--line);background:#0c0a08}
...
.words{display:flex;flex-wrap:wrap;gap:6px}.word{border:1px solid transparent;border-radius:6px;background:var(--soft);padding:7px 9px;color:var(--ink);cursor:pointer}
...
function renderStyles(){const project=output.project||{};const presets=output.presets||[];currentVersion=project.version;setStatus('Review styles');content.innerHTML='<div class="stack"><div><p class="eyebrow">Choose a look</p><h1>'+esc(project.title||'Caption styles')+'</h1><p class="muted" style="margin-top:7px">Compare the same caption in every style, then preview your choice.</p></div><div class="styles" id="styles" aria-label="Caption styles"></div><div class="row between"><p id="style-note" class="muted" aria-live="polite">Current style: '+esc(project.style&&humanize(project.style.preset)||'Custom')+'</p><button id="preview-style" class="btn primary">Preview this style</button></div></div>';const list=document.getElementById('styles');presets.slice(0,8).forEach(function(preset){const id=preset.preset;const button=document.createElement('button');button.className='style';button.setAttribute('aria-pressed',String(project.style&&project.style.preset===id));const preview=document.createElement('div');preview.className='style-preview '+id;preview.innerHTML=id==='karaoke'?'A few steps <b>reset</b> the mind':'A few steps reset the mind';const label=document.createElement('strong');label.textContent=humanize(id);button.append(preview,label);button.onclick=async function(){try{button.disabled=true;const result=await callTool('update_caption_project',{projectId:project.id,expectedVersion:currentVersion,ops:[{op:'set_preset',preset:id}]});const data=result.structuredContent||result;currentVersion=data.project.version;project.style=data.project.style;list.querySelectorAll('.style').forEach(function(item){item.setAttribute('aria-pressed','false')});button.setAttribute('aria-pressed','true');document.getElementById('style-note').textContent='Previewing '+humanize(id)}catch(error){showError(error)}finally{button.disabled=false}};list.appendChild(button)});document.getElementById('preview-style').onclick=function(){followUp('Render a free preview using the selected style for project '+project.id+'.')}}
