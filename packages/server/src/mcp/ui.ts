...
function render(next){if(!next||!Object.keys(next).length)return;if(next.project&&workspaceProject&&next.project.id===workspaceProject.id&&next.project.version<workspaceProject.version)return;rememberWorkspace();disposeWorkspace();stopApprovalTimer();output=next;if(next.project){stopPolling();renderWorkspace()}else if(next.quote||next.status==='checkout_required'||next.status==='render_started'){stopPolling();renderApproval()}else if(next.task){renderProgress()}else if(KIND==='approval'){stopPolling();renderApproval()}else if(KIND==='start'){stopPolling();renderStart()}else{showError(new Error('Project details are not ready. Ask ChatGPT to open this project again.'))}notifyHeight()}
${WIDGET_PROGRESS}
${WIDGET_WORKSPACE}
