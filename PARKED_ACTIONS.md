...
| 8 | **Enable live Whop checkout** | The AppAgentic-owned `ClipSubtitles` Whop business is verified and its business ID is stored in `mc-vault`. Production Secret Manager containers and least-privilege API bindings now exist, but no provider values have been added and `BILLING_PROVIDER` remains `none`. The current AppAgentic browser session is signed out of Whop, and the legal decisions in `docs/legal-paid-traffic-review.md` remain unresolved. | Complete the Whop sign-in/elevation handoff in the existing AppAgentic profile, capture the one-time production key directly into `mc-vault`, run `pnpm whop:catalog` then the reviewed `--apply`, register `/v1/billing/webhooks/whop`, and store the webhook secret plus nine read-back plan IDs in `mc-vault`/Secret Manager. Complete the legal review before enabling billing, then run the charged acceptance checklist in `docs/billing-launch.md`. |
