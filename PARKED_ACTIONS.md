...
| 8 | **Enable live Whop checkout** | The approved catalog, credit pools, workspace-bound checkout endpoint, authentic Standard Webhooks verification, idempotent and out-of-order-safe grants, web upgrade surfaces, provider-specific Terraform bindings, agent resume journey, and dry-run-first catalog reconciler are implemented and locally tested. Enabling payments still requires verified AppAgentic-owned Whop resources, secret versions, legal approval and real provider acceptance. | In the verified AppAgentic Whop account, run `pnpm whop:catalog` and review the dry-run; with exact provider-mutation approval run it once with `--apply`, register `/v1/billing/webhooks/whop` separately, and put the API key, account ID, webhook secret, and nine verified plan IDs in `mc-vault`/Secret Manager. Complete `docs/legal-paid-traffic-review.md`; then set `enable_billing = true`, deploy with exact approval, and run the charged checkout, signed replay, renewal/cancellation, portal and agent-resume checklist in `docs/billing-launch.md`. |
