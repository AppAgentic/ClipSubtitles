...
| 8 | **Enable live Whop checkout** | The exact AppAgentic-owned `ClipSubtitles` business, hidden product, six recurring plans, three top-ups, four-event production webhook, and all 12 provider values are verified in `mc-vault` and production Secret Manager. `BILLING_PROVIDER` intentionally remains `none` because the legal decisions in `docs/legal-paid-traffic-review.md` are unresolved. | Complete the legal review, enable the pinned Whop configuration, then run the controlled charged top-up, subscription, replay/idempotency, customer-portal, and agent-resume checks in `docs/billing-launch.md` before public payment traffic. |
| 11 | **Confirm paid-traffic alert delivery** | The operator email channel, two public uptime checks, and all five alert policies are live. A clearly labelled synthetic 5xx event was accepted by Cloud Logging on 2026-09-02, but the authenticated Gmail surface failed to load, so delivery to the human inbox has not yet been independently read back. | Confirm that the `joe@appagentic.dev` inbox received the ClipSubtitles Google Cloud incident notification. If it did not arrive, inspect the channel/provider delivery state before paid traffic. |

## Residual notes (not gates)

- The `ClipSubtitles Production Monthly` Google Cloud budget is live for the exact
  `clipsubtitles-production` project: £500 per month with actual-spend alerts at 50%, 90%, and
  100%, delivered to billing administrators and users. Budget notifications do not cap spend.
...
- The paid-traffic application release was merged and deployed on 2026-09-02.
  API, web and worker were canaried before a monitored 5% then 100% cutover;
  billing stayed disabled and the previous revisions remain available for
  rollback. See `docs/production-release-2026-09-02.md`.
