...
| 10 | **Choose the dashboard design direction** | Five GPT Image 2 concepts are preserved in `docs/design/dashboard-concepts/`. Implementing one before selection would bake subjective visual choices into the product. | Choose a direction or approve the recommended 2 + 3 + 4 synthesis; then create responsive wireframes and implement the new dashboard. |
