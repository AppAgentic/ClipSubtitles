...
| 12 | **Create the ClipSubtitles Gleap project** | The product integration is implemented and verified: support opens from the workspace header, Settings, and Help page; the generic floating widget is hidden; signed-in users receive safe workspace context; sensitive auth, billing, upload and project network data is excluded; and email remains the fallback. The isolated AppAgentic Gleap browser profile is currently signed out, so creating the app-specific project and tokens requires one human Google sign-in. | Sign in to Gleap once with `joe@appagentic.dev`, create/select the `ClipSubtitles` project, store its SDK token as `clipsubtitles-gleap-sdk-token` and its server API token as `clipsubtitles-gleap-api-token` in `mc-vault`, then inject the SDK token as `NEXT_PUBLIC_GLEAP_SDK_TOKEN` for the production web build and verify one identified test conversation. |
