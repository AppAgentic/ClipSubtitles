...
|---|------|---------------------------|------------------|
| 1 | **Resolve deployed Scribe Free-tier origin block or approve provider reordering** | Both the original and a newly created Speech-to-Text-only ElevenLabs key succeed locally but receive HTTP 401 from Cloud Run when pinned by explicit Secret Manager version. A bounded Cloud Run diagnostic using the same v4 key and worker identity confirmed ElevenLabs code `detected_unusual_activity` (trace `8e11eb5dce359fefbe13e6e440bf18b6`). This is the Free-tier shared/datacenter-IP abuse detector, not stale key material. Gemini fallback works end to end, but silently relying on fallback would contradict the benchmark-selected primary order. | Upgrade the ElevenLabs workspace and rerun the forced Cloud Run canary (the simplest supported fix), or provision unique static egress with VPC/Cloud NAT and retest. Alternatively, explicitly choose Gemini primary / the previously benchmarked partner route and rerun cost/quality acceptance. |
...
- The provider-order benchmark is resolved: two preserved runs over six real product voice
  ...
  as rollback while this provider gate is unresolved. A follow-up isolated
  ...
  `8e11eb5dce359fefbe13e6e440bf18b6`; the temporary diagnostic job was deleted
