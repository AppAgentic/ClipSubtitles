...
| 4 | **Dedicated GCP project + production deployment** | No dedicated GCP project, WorkOS tenant, secret versions, or approved live R2 bucket/token exists. The direct ElevenLabs credential is safely held in `mc-vault` but is not bound to any deployment. The async `DataStore` cutover and hardened direct-upload implementation are complete locally; `deploy_services=false` remains the Terraform default. Live R2 still needs exact-origin CORS plus a one-day `staging/` lifecycle safety net. | Create the dedicated AppAgentic GCP project, approve/provision R2, apply and read back `infra/r2/cors.production.json` plus the lifecycle rule in `docs/direct-upload-operations.md`, copy provider/WorkOS/R2/database credentials into Secret Manager without exposing them, and set `TRANSCRIPTION_PROVIDERS=elevenlabs,gemini` plus `ELEVENLABS_SCRIBE_MODEL=scribe_v2`. Run a direct Scribe staging smoke, then `terraform plan` with immutable image SHAs. Enable `deploy_services` only after plan review. Benchmark render CPU/RSS at 2/4/8 vCPU before raising worker concurrency above 1. |
