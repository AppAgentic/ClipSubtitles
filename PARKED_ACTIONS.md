...
| 8 | **Enable live Whop checkout** | The catalog and secrets are provisioned, and dated public policies are drafted on the legal-review branch. Live review found incomplete Whop identity/business verification and business address; no uploaded legal PDFs/required acceptance; no `refund.created` or `dispute.created` ledger handling; an unverified ElevenLabs model-improvement setting; and outstanding ICO/accountant/solicitor checks. `BILLING_PROVIDER` intentionally remains `none`. | Complete the seven blockers in `docs/legal-review-2026-09-03.md`, deploy the approved policies, enable the pinned Whop configuration, then run the charged purchase/refund/replay/portal/agent-resume checks in `docs/billing-launch.md`. |
