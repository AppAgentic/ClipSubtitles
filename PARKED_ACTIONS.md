...
| 8 | **Enable live Whop checkout** | The approved catalog, credit pools, workspace-bound checkout endpoint, authentic Standard Webhooks verification, idempotent and out-of-order-safe grants, web upgrade surfaces, provider-specific Terraform bindings, and agent resume journey are implemented and locally tested. Enabling payments still requires verified AppAgentic-owned Whop resources, secret versions, and real provider acceptance. | In the verified AppAgentic Whop account, create/read back the Creator/Pro/Studio monthly and annual plans plus three top-up products matching `BILLING_CATALOG`; register `/v1/billing/webhooks/whop`; put the API key, account ID, webhook secret, and nine plan IDs in `mc-vault`/Secret Manager; set `enable_billing = true`; deploy with exact approval; then run the real checkout, signed replay, renewal/cancellation, portal and agent-resume checklist in `docs/billing-launch.md`. Complete legal review of `/terms` and `/privacy` before accepting money. |
