...
The separate consent-only patch gates optional browser collection and advertising attribution, remembers separate choices, supports withdrawal/cross-tab changes, and loads Gleap only after deliberate Help. Local desktop/mobile flows passed; original full check passed411tests plus lint/types/build. This release is [PR15](https://github.com/AppAgentic/ClipSubtitles/pull/15), updated commit `03abbb7`;18localbrowser journeys passed and CI33996161485 is pending. It requires its own serving-revision receipt; deployment is not yet claimed here. It excludes unresolved age/Terms wording. Existing ElevenLabs/Gemini processing must remain accurately disclosed until the runtime changes. Improve models ON means no provider no-training assurance is justified.
...
- Final production consent and policy readback, plus exact final annotation/metadata alignment after any provider change.
