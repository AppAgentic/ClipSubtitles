  ...
  if (pages.length === 0) {
    return (
      <div className="px-4 py-6 text-[12px] text-ink-mute">
        No caption pages yet. Generate captions to populate this list.
      </div>
    );
  }
  ...
  return (
    <ul ref={listRef} className="min-h-0 flex-1 overflow-y-auto" aria-label="Caption pages">
      {pages.map((p, i) => {
        ...
        const worst = issues.some((x) => x.severity === 'error')
          ? 'error'
          : issues.length
            ? 'warning'
            : null;
        ...
        return (
          <li
            key={p.id}
            data-page={p.id}
            className={`group relative border-b border-line/60 transition-colors ${selected ? 'bg-panel-2' : 'hover:bg-panel-2/60'}`}
          >
            <div
              className={`absolute left-0 top-0 h-full w-[2px] transition-colors ${active ? 'bg-signal' : 'bg-transparent'}`}
            />
            <button
              type="button"
              onClick={() => onSelect(p)}
              aria-current={selected ? 'true' : undefined}
              className="w-full px-4 py-2.5 text-left outline-none focus-visible:bg-panel-2"
            >
              <span className="flex items-center justify-between gap-2">
                ...
                <span className="flex items-center gap-1.5">
                  {p.manual ? (
                    <span className="mono text-[9px] uppercase tracking-[0.12em] text-info">
                      manual
                    </span>
                  ) : null}
                  <span
                    className={`mono text-[10px] ${worst === 'error' ? 'text-danger' : worst === 'warning' ? 'text-warn' : 'text-ink-mute'}`}
              ...
              <span
                className={`mt-1 block whitespace-pre-line pr-16 text-[13px] leading-snug ${active ? 'text-ink' : 'text-ink-dim'}`}
              >
            ...
            {i < pages.length - 1 ? (
              <button
                ...
                className="absolute right-3 bottom-2 rounded-full border border-line-strong bg-bg-elev px-2 py-1 text-[10px] text-ink-mute opacity-0 transition-opacity hover:border-signal hover:text-signal focus-visible:opacity-100 group-hover:opacity-100 group-focus-within:opacity-100"
              >
                merge ↓
              </button>
            ) : null}
