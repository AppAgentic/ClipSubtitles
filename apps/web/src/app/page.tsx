...
import { Button, Chip, EmptyState, LinkButton, Panel, Progress, statusTone } from '@/components/ui/primitives';
...
function Library() {
  ...
  const activeRenderCount = tasks.filter((t) => t.kind === 'render_export' && (t.status === 'queued' || t.status === 'running')).length;
  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
      <div className="min-w-0">
        <div className="rise mb-5 flex items-end justify-between">
          <div>
            ...
            <p className="text-[13px] text-ink-mute">Every project, task, and export your agents and you created — recoverable here.</p>
          </div>
        ...
        {projects === null ? (
          ...
          <div className="rise rise-1">
            <EmptyState
              title="Drop a clip. Get captions you can trust."
              body="Upload a short video (or paste a public URL). Generation runs as a durable task; you review word timing and style here, then approve the render cost."
              actions={
                <>
                  <LinkButton href="/projects/new" variant="primary">
                    Upload a clip
            ...
            <table className="w-full text-left text-[13px]">
              <thead className="text-[10px] uppercase tracking-[0.14em] text-ink-mute">
                <tr className="border-b border-line">
                  ...
                  <th className="px-3 py-2.5 text-right font-medium">Ver</th>
              ...
              <tbody>
                {projects.map((p) => (
                  <tr key={p.id} className="group border-b border-line/60 transition-colors last:border-b-0 hover:bg-panel-2/60">
                    <td className="px-4 py-3">
                      <Link href={`/projects/${p.id}`} className="font-medium text-ink hover:text-signal">
                        {p.title}
                      </Link>
                      <div className="mono mt-0.5 text-[11px] text-ink-mute">{p.id}</div>
                    </td>
                    <td className="px-3 py-3">
                      <Chip tone={statusTone(p.status)} dot={p.status === 'transcribing' || p.status === 'importing'}>
                        {titleCase(p.status)}
                      </Chip>
                    </td>
                    <td className="mono px-3 py-3 text-right text-ink-dim">{p.durationMs !== undefined ? timecode(p.durationMs, false) : '—'}</td>
                    <td className="mono px-3 py-3 text-right text-ink-dim">{p.pageCount}</td>
                    <td className="mono px-3 py-3 text-right text-ink-dim">v{p.version}</td>
                    <td className="mono px-3 py-3 text-right text-ink-mute">{relativeTime(p.updatedAt)}</td>
                    <td className="px-3 py-3 text-right">
                      <div className="flex justify-end gap-1 opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">
                        <LinkButton href={`/projects/${p.id}`} size="sm">
                          Open
                        </LinkButton>
        ...
        {projects && projects.length > 0 && fixtures.length > 0 ? (
          <div className="mt-4 flex flex-wrap items-center gap-2 text-[12px] text-ink-mute">
            ...
            {fixtures.map((f) => (
              <Button key={f.id} size="sm" variant="subtle" onClick={() => void useDemo(f.id)} loading={busy === f.id}>
                {f.title}
              </Button>
            ))}
          </div>
      ...
      <aside className="flex flex-col gap-4">
        <Panel title="Tasks" className="rise rise-2" aside={hasActive ? <Chip tone="signal" dot>live</Chip> : null}>
...
export function TaskList({ tasks, onChanged, compact = false }: { tasks: Task[]; onChanged?: () => void; compact?: boolean }) {
  const toast = useToast();
  ...
  if (tasks.length === 0) return <div className="px-4 py-3 text-[12px] text-ink-mute">No tasks yet.</div>;
  return (
        ...
        return (
          <li key={t.id} className="px-4 py-2.5">
            <div className="flex items-center justify-between gap-2">
              <span className="text-[12px] font-medium text-ink">{titleCase(t.kind)}</span>
            ...
            <div className="mono mt-1 flex items-center justify-between text-[11px] text-ink-mute">
              <span>{t.projectId ? <Link href={`/projects/${t.projectId}`} className="hover:text-signal">{t.projectId.slice(0, 14)}…</Link> : t.id}</span>
              <span>{t.stage ?? relativeTime(t.updatedAt)}</span>
            </div>
            {active ? (
              <div className="mt-2 flex items-center gap-2">
                <Progress value={t.progress} className="flex-1" />
                <button type="button" onClick={() => void cancel(t)} className="text-[11px] text-ink-mute hover:text-danger">
                  cancel
                </button>
              </div>
            ) : null}
            {t.error ? (
              <div className="mt-1 text-[11px] text-danger">
                ...
                {t.error.errorRef ? <span className="mono text-ink-mute"> · {t.error.errorRef}</span> : null}
              </div>
            ) : null}
          </li>
        );
...
export function ExportList({ exports }: { exports: Export[] }) {
  if (exports.length === 0) return <div className="px-4 py-3 text-[12px] text-ink-mute">Nothing rendered yet.</div>;
  return (
    <ul className="divide-y divide-line/70">
      {exports.map((e) => (
        <li key={e.id} className="flex items-center justify-between gap-3 px-4 py-2.5">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="mono rounded border border-line-strong px-1.5 py-[1px] text-[10px] uppercase text-ink-dim">{e.kind}</span>
              <span className="truncate text-[12px] text-ink">{e.fileName}</span>
            </div>
            <div className="mono mt-0.5 text-[11px] text-ink-mute">
              {bytes(e.bytes)} · v{e.projectVersion} · expires {relativeTime(e.expiresAt)}
            </div>
          </div>
          {e.downloadUrl ? (
            <a href={`${e.downloadUrl}&download=1`} className="shrink-0 text-[12px] text-signal hover:text-signal-soft">
              Download
            </a>
          ) : (
            <span className="text-[11px] text-ink-mute">purged</span>
          )}
        </li>
      ))}
    </ul>
